...
const infer = {
      ...
      fallback: {
        ...
        title: 'Highlighted Feature',
         ...
         const title = resolveValue("title", highlightedFeature.preview.select, x);         const subtitle = resolveValue("subtitle", highlightedFeature.preview.select, x);         const media = resolveValue("media", highlightedFeature.preview.select, x);         return handleReplacements({           title: infer.singletonTitle || title || undefined,           subtitle: subtitle ? subtitle : infer.fallback["title"],           media: media || infer.icon         }, x, infer.fallback);       },
  },
}