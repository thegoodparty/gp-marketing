...
export const pricingPlanFeatureListItem = {
	...
	preview: {
		...
		prepare: x => {
			...
			const title = resolveValue('title', pricingPlanFeatureListItem.preview.select, x);
			const subtitle = resolveValue('subtitle', pricingPlanFeatureListItem.preview.select, x);
			const media = resolveValue('media', pricingPlanFeatureListItem.preview.select, x);
			return handleReplacements(
				{
					title:
						infer.singletonTitle || typeof title === 'string'
							? title
							: Array.isArray(title)
								? toPlainText(title as Parameters<typeof toPlainText>[0])
								: infer.name,
