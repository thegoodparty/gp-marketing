import { getCandidacies, getPersonsByIds } from '~/lib/electionsApi';
...
/**
 * The hero's ballot rows for a race. Returns `undefined`, not `[]`, when the
 * fetch fails, so the card hides rather than publishing "0 candidates" for a
 * race we simply could not read.
 */
export async function loadPositionHeroCandidates(raceSlug: string): Promise<ElectionsPositionHeroCandidate[] | undefined> {
	let candidacies: CandidacyItem[];
	try {
		candidacies = await getCandidacies({ raceSlug });
	} catch {
		return undefined;
	}
