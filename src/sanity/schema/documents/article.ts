...
const infer = {
      ...
      fallback: {
        ...
        title: 'Articles',
         ...
         const title = resolveValue("title", article.preview.select, x);         const subtitle = resolveValue("subtitle", article.preview.select, x);         const media = resolveValue("media", article.preview.select, x);         return handleReplacements({           title: infer.singletonTitle || title || undefined,           subtitle: subtitle ? subtitle : infer.fallback["title"],           media: media || infer.icon         }, x, infer.fallback);       },
  },
  groups: [
    {
      title: 'Overview',
    ...
    documentSlugs: [
      {
        ...
        slugSources: [
          'editorialOverview.field_editorialTitle',
        ],
      },
    ],
    single: false,
