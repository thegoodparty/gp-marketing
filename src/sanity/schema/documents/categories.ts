...
const infer = {
      ...
      fallback: {
        ...
        title: 'Blog Categories',
         ...
         const title = resolveValue("title", categories.preview.select, x);         const subtitle = resolveValue("subtitle", categories.preview.select, x);         const media = resolveValue("media", categories.preview.select, x);         return handleReplacements({           title: infer.singletonTitle || title || undefined,           subtitle: subtitle ? subtitle : infer.fallback["title"],           media: media || infer.icon         }, x, infer.fallback);       },
  },
  groups: [
    {
      title: 'Overview',
