...
const infer = {
      ...
      fallback: {
        ...
        title: 'Blog Topics',
         ...
         const title = resolveValue("title", topics.preview.select, x);         const subtitle = resolveValue("subtitle", topics.preview.select, x);         const media = resolveValue("media", topics.preview.select, x);         return handleReplacements({           title: infer.singletonTitle || title || undefined,           subtitle: subtitle ? subtitle : infer.fallback["title"],           media: media || infer.icon         }, x, infer.fallback);       },
  },
  groups: [
    {
      title: 'Overview',
  ...
  options: {
    ...
    channels: {
      goodpartyOrg: '/blog/topic/:slug',
    },
    documentSlugs: [
      {
        slugField: 'tagOverview.field_slug',
        slugSources: [
          'tagOverview.field_name',
        ],
      },
    ],
    single: false,
