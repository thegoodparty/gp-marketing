...
export const ctaAssets = {
	...
	preview: {
		...
		prepare: x => {
			...
			const title = resolveValue('title', ctaAssets.preview.select, x);
			const subtitle = resolveValue('subtitle', ctaAssets.preview.select, x);
			const media = resolveValue('media', ctaAssets.preview.select, x);
