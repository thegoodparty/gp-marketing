...
export const component_twoUpCardBlock = {
	...
	preview: {
		select: {
			title: 'twoUpCardBlockOne.field_twoUpCardBlockCardType',
			subtitle: 'twoUpCardBlockTwo.field_twoUpCardBlockCardType',
			_type: '_type',
		},
		prepare: x => {
			const infer = {
			...
			const title = resolveValue('title', component_twoUpCardBlock.preview.select, x);
			const subtitle = resolveValue('subtitle', component_twoUpCardBlock.preview.select, x);
			const media = resolveValue('media', component_twoUpCardBlock.preview.select, x);
			return handleReplacements(
				{
					title: infer.singletonTitle || title + ' & ' + subtitle || undefined,
					subtitle: infer.fallback['title'],
