...
export function handleReplacements(i: Record<string, unknown>, _x: Record<string, any>, fallback: any, _title?: string) {
	const p = checkReturnI18n(i, primaryLanguage);
	if (typeof fallback.previewTitle === 'string' && String(fallback.previewTitle).startsWith('*')) {
		p.title = String(fallback.previewTitle).replace('* ', '').replace('*', '');
	}
	if (typeof fallback.previewTitle === 'string' && String(fallback.previewTitle).startsWith('_type')) {
		p.title = fallback.title;
	}
	if (typeof fallback.previewSubTitle === 'string' && String(fallback.previewSubTitle).startsWith('*')) {
		p.subtitle = String(fallback.previewSubTitle).replace('* ', '').replace('*', '');
	}
	if (typeof fallback.previewSubTitle === 'string' && String(fallback.previewSubTitle).startsWith('_type')) {
