...
import { ChangeIndicator, PatchEvent, type ReferenceDefinition, set, type StringInputProps } from 'sanity';
		...
		...sitesList.map(([siteId]) => {
			return {
				...
				to: documentSchema.flatMap(doc => {
					const isInChannel = 'channels' in doc.options && doc.options.channels && siteId in doc.options.channels;
					if (!isInChannel) {
						return [];
					}
...
function LinkSiteInput(props: StringInputProps) {
	...
	useEffect(() => {
		if (!props.value && items.at(0)?.value) {
			onChange(PatchEvent.from(set({ ['site']: items.at(0)!.value }, props.path.slice(0, -1))));
		}
	}, [props.value]);
	return (
		<ChangeIndicator path={props.path} isChanged={false} hasFocus={!!props.focused} style={{ maxWidth: 'fit-content' }}>
			<Autocomplete
				{...props.elementProps}
				openButton
				...
				radius={0}
				prefix={<PreCon>{currentSiteData?.icon({})}</PreCon>}
				style={{ maxWidth: 'fit-content' }}
				fontSize={[1]}
				onChange={nextItem => {
					if (nextItem !== props.value) {
						onChange(PatchEvent.from(set({ ['site']: nextItem }, props.path.slice(0, -1))));
					}
				}}
