...
export const imageBlockItem = {
	...
	preview: {
		...
		prepare: x => {
			...
			const title = resolveValue('title', imageBlockItem.preview.select, x);
			const subtitle = resolveValue('subtitle', imageBlockItem.preview.select, x);
			const media = resolveValue('media', imageBlockItem.preview.select, x);
